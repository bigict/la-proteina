...
def concat_main(args):
    ...
    for db_in in args.db_in:
        in_dir, in_name, in_seq_id = dataset_file_split(db_in)
        assert out_dir / f"{out_name}.csv" != in_dir / f"{in_name}.csv", (
            f"{args.db_out} == {db_in}"
        )

    out_dir.mkdir(parents=True, exist_ok=True)

    df_pdb_csv, df_pdb_csv_fieldnames = [], []
    seq_df_pdb_fasta, cluster_seqid_fasta, cluster_seqid_tsv = [], [], []

    for db_in in args.db_in:
        if args.verbose:
            print(f"Read database: {db_in} ...")
        ...
        with open(in_dir / f"{in_name}.csv", "r") as f:
            ...
            df_pdb_csv_fieldnames += [
                field for field in reader.fieldnames if field not in df_pdb_csv_fieldnames
            ]
            df_pdb_csv += [row for row in reader]
        ...
        cluster_seqid_fasta += [
            record for record in SeqIO.parse(
                in_dir / f"cluster_seqid_{in_seq_id}_{in_name}_test.fasta", "fasta"
            )
        ]
        ...
        with open(in_dir / f"cluster_seqid_{in_seq_id}_{in_name}_test.tsv", "r") as f:
            for line in filter(lambda x: x, map(lambda x: x.strip(), f)):
                ...
                cluster_seqid_tsv.append((rep_id, pdb_id))

    with open(out_dir / f"{out_name}.csv", "w") as f:
        writer = csv.DictWriter(f, fieldnames=df_pdb_csv_fieldnames)
        writer.writeheader()
        writer.writerows(df_pdb_csv)
    SeqIO.write(seq_df_pdb_fasta, out_dir / f"seq_{out_name}.fasta", "fasta-2line")
    SeqIO.write(
        cluster_seqid_fasta,
        out_dir / f"cluster_seqid_{out_seq_id}_{out_name}_test.fasta", "fasta-2line"
    )
    with open(out_dir / f"cluster_seqid_{out_seq_id}_{out_name}_test.tsv", "w") as f:
        for rep_id, pdb_id in cluster_seqid_tsv:
            f.write(f"{rep_id}\t{pdb_id}\n")

    if args.verbose:
        print(f"Output: {args.db_out}")


def concat_add_argument(parser):
    parser.add_argument(
        "-o", "--db_out", type=pathlib.Path, default="./pdb_train", help="database output"
    )
    parser.add_argument("db_in", type=pathlib.Path, nargs="+", help="dataset input")
    return parser
...
if __name__ == "__main__":
    ...
    commands = {
        "concat": (concat_main, concat_add_argument),
